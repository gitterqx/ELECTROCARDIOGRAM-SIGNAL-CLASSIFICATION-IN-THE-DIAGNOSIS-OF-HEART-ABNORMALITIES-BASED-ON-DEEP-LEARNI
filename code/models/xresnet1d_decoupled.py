...
from models.basic_conv1d_decoupled import create_head1d, create_head1d_decoupled
from models.basic_conv1d import create_priMask_ST
...
class XResNet1d(nn.Sequential):
    ...
    def __init__(self, block, expansion, layers, p=0.0, input_channels=3, num_classes=1000, stem_szs=(32,32,64),kernel_size=5,kernel_size_stem=5,
                 widen=1.0, sa=False, act_cls=nn.ReLU, lin_ftrs_head=None, ps_head=0.5, bn_final_head=False, bn_head=True, act_head="relu", concat_pooling=True, if_train=True, **kwargs):
        ...
        priM = create_priMask_ST(num_classes)
...
class XResNet1d_decoupled(nn.Sequential):
    @delegates(ResBlock)
    def __init__(self, block, expansion, layers, p=0.0, input_channels=3, num_classes=1000, stem_szs=(32, 32, 64),
        ...
        head = create_head1d_decoupled(block_szs[-1] * expansion, nc=num_classes, lin_ftrs=lin_ftrs_head, div_lin_ftrs=div_lin_ftrs_head, ps=ps_head,
                             bn_final=bn_final_head, bn=bn_head, act=act_head, concat_pooling=concat_pooling,
                             if_train=if_train)
        priM = create_priMask_ST(num_classes)
        
        super().__init__(
            *priM, *stem, nn.MaxPool1d(kernel_size=3, stride=2, padding=1),
            *blocks,
            head,
        )
...
#xresnets
def _xresnet1d_decoupled(expansion, layers, **kwargs):
    return XResNet1d_decoupled(ResBlock, expansion, layers, **kwargs)
...
def xresnet1d18_decoupled (**kwargs): return _xresnet1d_decoupled(1, [2, 2,  2, 2], **kwargs)
def xresnet1d34_decoupled (**kwargs): return _xresnet1d_decoupled(1, [3, 4,  6, 3], **kwargs)
def xresnet1d50_decoupled (**kwargs): return _xresnet1d_decoupled(4, [3, 4,  6, 3], **kwargs)
def xresnet1d101_decoupled(**kwargs): return _xresnet1d_decoupled(4, [3, 4, 23, 3], **kwargs)
def xresnet1d152_decoupled(**kwargs): return _xresnet1d_decoupled(4, [3, 8, 36, 3], **kwargs)
def xresnet1d18_deep_decoupled  (**kwargs): return _xresnet1d_decoupled(1, [2,2,2,2,1,1], **kwargs)
def xresnet1d34_deep_decoupled  (**kwargs): return _xresnet1d_decoupled(1, [3,4,6,3,1,1], **kwargs)
def xresnet1d50_deep_decoupled  (**kwargs): return _xresnet1d_decoupled(4, [3,4,6,3,1,1], **kwargs)
def xresnet1d18_deeper_decoupled(**kwargs): return _xresnet1d_decoupled(1, [2,2,1,1,1,1,1,1], **kwargs)
def xresnet1d34_deeper_decoupled(**kwargs): return _xresnet1d_decoupled(1, [3,4,6,3,1,1,1,1], **kwargs)
def xresnet1d50_deeper_decoupled(**kwargs): return _xresnet1d_decoupled(4, [3,4,6,3,1,1,1,1], **kwargs)
