...
from models.basic_conv1d_decoupled import AdaptiveConcatPool1d,create_head1d,create_head1d_decoupled
...
class Inception1d(nn.Module):
    ...
    def __init__(self, num_classes=2, input_channels=8, kernel_size=40, depth=6, bottleneck_size=32, nb_filters=32, use_residual=True,lin_ftrs_head=None, ps_head=0.5, bn_final_head=False, bn_head=True, act_head="relu", concat_pooling=True):
        ...
        layers = [pri]
        inception = InceptionBackbone(input_channels=input_channels, kss=kernel_size, depth=depth, bottleneck_size=bottleneck_size, nb_filters=nb_filters, use_residual=use_residual)
...
class Inception1d_decoupled(nn.Module):
    '''inception time architecture'''

    def __init__(self, num_classes=2, input_channels=8, kernel_size=40, depth=6, bottleneck_size=32, nb_filters=32,
                 use_residual=True, lin_ftrs_head=None, div_lin_ftrs_head=None, ps_head=0.5, bn_final_head=False, bn_head=True, act_head="relu",
                 concat_pooling=True, if_train=True):
        super().__init__()
        assert (kernel_size >= 40)
        ...
        n_ks = len(kernel_size) + 1
        # head
        head = create_head1d_decoupled(n_ks * nb_filters, nc=num_classes, lin_ftrs=lin_ftrs_head, div_lin_ftrs=div_lin_ftrs_head, ps=ps_head,
...
def inception1d_decoupled(**kwargs):
    """Constructs an Inception model
    """
    return Inception1d_decoupled(**kwargs)
