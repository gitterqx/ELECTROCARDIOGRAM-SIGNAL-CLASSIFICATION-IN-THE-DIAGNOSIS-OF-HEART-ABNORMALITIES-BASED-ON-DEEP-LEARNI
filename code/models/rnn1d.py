...
import math
from models.basic_conv1d_decoupled import DivOutLayer
...
class RNN1d_decoupled(nn.Sequential):
    def __init__(self, input_channels, num_classes, lstm=True, hidden_dim=256, num_layers=2, bidirectional=False,
                 ps_head=0.5, act_head="relu", lin_ftrs_head=None, div_lin_ftrs_head=None, bn=True, if_train=True):
        pri = priMaskST(num_classes)
        layers_tmp = [pri]
        # bs, ch, ts -> ts, bs, ch
        # LSTM
        ...
        nf = 3 * hidden_dim if bidirectional is False else 6 * hidden_dim
        lin_ftrs_head = [nf, num_classes] if lin_ftrs_head is None else [nf] + lin_ftrs_head + [num_classes]
        div_lin_ftrs_head = [nf, num_classes] if lin_ftrs_head is None else [nf] + div_lin_ftrs_head + [1]
        ps_head = listify(ps_head)
        if len(ps_head) == 1:
            ps_head = [ps_head[0] / 2] * (len(lin_ftrs_head) - 2) + ps_head
        em_actns = [nn.ReLU(inplace=True) if act_head == "relu" else nn.ELU(inplace=True)] * (len(lin_ftrs_head) - 2) + [None]
        div_actns = [nn.ReLU(inplace=True) if act_head == "relu" else nn.ELU(inplace=True)] * (len(div_lin_ftrs_head) - 3) + [None, None]

        layers_head.append(DivOutLayer(em_structure=lin_ftrs_head, div_structure=div_lin_ftrs_head, bn=bn, drop_rate=ps_head, em_actns=em_actns, div_actns=div_actns, cls_num=num_classes, metric_out_dim=div_lin_ftrs_head[-2], if_train=if_train))
