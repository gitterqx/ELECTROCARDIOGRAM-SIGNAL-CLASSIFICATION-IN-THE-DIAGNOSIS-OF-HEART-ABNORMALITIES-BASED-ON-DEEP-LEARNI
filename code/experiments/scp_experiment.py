...
class SCP_Experiment():
    ...
    def get_priMatrix(self,model_description):
        ...
        X = [l.astype(np.float32) for l in self.X_val]
        y_dummy = [np.ones(model.num_classes,dtype=np.float32) for _ in range(len(X))]
        
        learn = model._get_learner(X,y_dummy,X,y_dummy,if_train = False)
        learn.load(model.name)
        #learn.load('optimal')
        
        count=0
        
        for pri in learn.model.named_parameters():
            if count==0:
                return pri[1].data.cpu()
    ...
    def perform(self):
        ...
        for model_description in self.models:
            ...
            #model.predict_opt(self.X_test).dump(mpath+'y_test_pred_opt.npy')
            
        
    def perform_wo_train(self):
    ...
    def evaluate(self, n_bootstraping_samples=100, n_jobs=20, bootstrap_eval=False, dumped_bootstraps=True):
        ...
        for model_description in self.models:
            ...
            #y_test_pred_opt = np.load(mpath+'y_test_pred_opt.npy', allow_pickle=True)
            ...
            #te_df_point_opt = utils.generate_results(range(len(y_test)), y_test, y_test_pred_opt, thresholds)
